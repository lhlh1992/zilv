...
export class SystemManageDicitemComponent implements OnInit {
  ...
  @ViewChild('st', { static: true }) itemst: STComponent;
  ...
  // 获取字典类型列表
  query() {
    this.config.post(this.config.url + 'dic/dictypeList', {}).subscribe((res: any) => {
      this.data = res;
    });
  }
  // 字典类型列表点击事件
  ...
  // 获取字典配置列表
  queryItem(code) {
    this.config.post(this.config.url + 'dic/dicitemList', { code }).subscribe((res: any) => {
      this.itemData = res;
    });
  }
  // 新增字典类型打开模态框
  ...
  handleOk(): void {
    ...
    if (this.typeAddorEdit == 'add') {
      this.config
        .post(this.config.url + 'dic/insertDictype', {
          dic_type_code: this.dic_type_code,
          dic_type_name: this.dic_type_name,
        })
        .subscribe((res: any) => {
          if (res == 1) {
      ...
      this.config
        .post(this.config.url + 'dic/updateDictype', {
          dic_type_code: this.dic_type_code,
          dic_type_name: this.dic_type_name,
          id: this.id,
        })
  ...
  handledelete() {
    this.config.post(this.config.url + 'dic/deleteDictype', { id: this.id }).subscribe((res: any) => {
      if (res == 1) {
  ...
  handleOkItem(): void {
    if (this.typeAddorEditItem == 'add') {
      this.config
        .post(this.config.url + 'dic/insertDicItem', {
          dicItemCode: this.dicItemCode,
          dicItemName: this.dicItemName,
          dicTypeId: this.dictypeCode,
        })
        .subscribe((res: any) => {
          if (res == 1) {
            this.msg.success('添加成功');
      ...
      this.config
        .post(this.config.url + 'dic/updateDicitem', {
          dicItemCode: this.dicItemCode,
          dicItemName: this.dicItemName,
          id: this.itemID,
        })
  ...
  handledeleteItem() {
    this.config.post(this.config.url + 'dic/deleteDicitem', { id: this.itemID }).subscribe((res: any) => {
      if (res == 1) {
        this.msg.success('删除成功');
        this.isVisibledeleteItem = false;
        // 刷新列表
